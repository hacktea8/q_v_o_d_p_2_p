...
class CloudsearchClient {
   ...
   * @var string
   */
  private $host = 'http://opensearch.etao.com';
  ...
  /**
   * 构造函数。
   * @param string $clientId 用户的client_id，从网站中可以获得此信息。
   * @param string $clientSecret 用户的client_secret，从网站中可以获得此信息。
   * @param array $opts 一些可选信息，包含：
   *     string version 当前sdk的版本信息，默认为$this->version。
   *     string host 指定请求的host地址，默认为$this->host。
   *     string gzip 指定返回的结果用gzip压缩。默认为$this->gzip。
   */
  public function __construct($clientId, $clientSecret, $opts = array()) {
    ...
    $this->baseURI = rtrim($this->host, '/') . '/' . $this->version . '/api';
  }
   ...
   * @param string $method 当前请求的方法，默认为self::METHOD。
